...
def display_recommendations(response):
    ...
    lbl_track_name= tk.Label(master= frm_recommendations, text = 'Artist Name')
    lbl_track_name= tk.Label(master= frm_recommendations, text = 'Play It')
    lbl_track_name.grid(row=0, column=0)
    lbl_artist_name.grid(row=0, column=1)
    lbl_play_it.grid(row=0, column=2)
    for idx, track in enumerate(response['track']):
        lbl_track_name_recommended = create_label(track['name'])
        lbl_track_name_recommended.grid(row= idx+1, column = 0)
        lbl_artist_name_recommended= create_label(track['artist'][0]['name'])
...
def get_recommendations():
    ...
    sp= spotipy.Spotify(client_credentials_manager = SpotifyClientCredentials("366d4ba51d7c421581c1ffb26d90d44b","ebc35744ce2f46ef875ae7b32f33ed90"))
